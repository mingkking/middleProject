...
<title>managerQuestion.jsp</title>
...
<body>
						...
						<div class="card-body">
							<table class="table">
								...
								<c:forEach items="${question }" var="question">
									<tr>
										<td>${ question.qNo}</td>
										<td align="question"><a href="${path }/managerGetQuestion?qNo=${question.qNo}">
											${question.qTitle }</a></td>
										<td>${question.id }</td>
    ...
    <c:forEach begin="${paging.startPage}" end="${paging.endPage}" step="1" varStatus="loop">
        ...
        <c:otherwise>
        	<a href="${path}/managerQuestion?pageNum=${loop.index}&searchCondition=${searchCondition}&searchKeyword=${searchKeyword}">${loop.index}</a>
        </c:otherwise>
        </c:choose>
    </c:forEach>
